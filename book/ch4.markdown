...
И только две вещи мрачно поглядывают из под тёмных капюшонов: `nil` («ничто») и `false` («фальш»).
...
`if` это как престижный клуб. Негативные элементы не пропускаются в защищённый кусок программы.
...
А ещё, у него есть знакомые в клубе `if`, поэтому он заходит через служебный вход.
«Тук–тук, можно к вам?» — «А, это ты, `true`, заходи!»
(Это всё потому, что у него *очень* искренне лицо.)

    print "Заходи!" if true # эти две строчки...
    print "Заходи!"         # ...работают одинаково
