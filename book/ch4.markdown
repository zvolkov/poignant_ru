...
И только две вещи мрачно поглядывают из под тёмных капюшонов: `nil` (англ. «ничто») и `false` (англ. «нет, это не так»).
...
Сегодня в столовке я встретил `true` (англ. «да, это так»).
...
Дочитав главу ты удаляешься в спальню, немного поразмыслить над смыслом прочитанного. Твой уютный альков отгорожен занавесочкой и завален подушками.
Ты залазишь на кучу подушек и, скрестив ноги, начинаешь созерцать. За окном виднеются громады заводских труб, изрыгающих в небо столбы крутящейся копоти.
Перепутанные ленточки дорог покрыты искорками вечернего автотранспорта, мигающего как огромный уставший глаз…

Как чудесно! В молоке горизонта тонет яичный желток заката, заливая город всеми оттенками заварного крема с ванилью…
...
> _55 тыс. обезьян — с такой поддержкой можно сделать всё! Захватить Грузию, сравнять с лицом земли и построить развлекательный центр **на всю территорию**. Трек для гонок на картингах, казино для вегитарианцев, сеть ресторанов быстрого питания. Главное блюдо — куринные крылышки. Специальный соус: Хрен с кодеином! «Птичечкин звездосоус "Сияющий" — любимая пища звездоликих обезьян». Не забыть запатентовать торговую марку, слоган и логотип… Бизнес–модель: франчайзинг… Инструкция для продавцов: «При необходимости выдать клиенту сдачу, поместите её внутрь жабы на своей правой руке. При отсутствии сдачи, в жабу следует поместить чек». Всё должно быть по правилам: Пиццу продаём дешевле себестоимости, деньги отобъём на напитках… Акция: собери надпись из букв под крышечкой…_

Вау, вот это идеи попёрли! Пришлось шлёпнуть себя по лицу, чтобы попустило! 
Эти злобные планы надо спрятать подальше, а еще лучше зашифровать. Давай так: я начинаю писать шифровальную программу, а ты пока проверь окно — нет ли там слежки.


### Шифрование с помощью переворачивания

    print "Пиши и не бойся ЦРУ: "
    idea_naoborot = gets.reverse

Понарошку, это программа для учёта злобных планов. Она просит пользователя ввести секретную запись, и шифрует её путем переставления букв задом–наперёд. Метод `gets` (англ. get string — «получить строчку»), как и `print`, это **ядерный метод**. Это значит что он **встроен прямо в Руби** и для его использования не нужно подключать никакую библиотечку. Когда программа доходит до `gets`, она останавливается и ждёт, пока пользователь что–нибудь напишет и нажмёт «Enter». Как только пользователь нажал «Enter», метод `gets` перестаёт следить за клавиатурой и выдаёт на выход строчку напечатанную пользователем.
...
Чё–то мне кажется `reverse` не поканает. Достаточно поднести к зеркалу «илмез моцил с ьтянварс и юизурГ ьтитавхаЗ» и всё становится понятно. Не успеют наши звездоликие бойцы прибыть в Цхинвали, как ЦРУ позвонит грузинским пограничникам и операция будет провалена.

Помойму, это заглавные буквы всё портят. А ну–ка если мы сначала сделаем **все** буквы заглавными:

     idea_naoborot = gets.upcase.reverse

Ну вот, гораздо лучше, правда?


### Повторенье мать ученья

И вот, ты вручаешь мне общую тетрадь, сплошь исписанную мелким неразборчивым почерком. На десятой минуте разбирания кракозяблей я наконец начинаю улавливать какие–то закономерности. Одни и те же слова встречаются снова и снова: звездоликие обезьяны, Грузия, напалм. Повторяющиеся словосочетания… «Покажем где лобстеры зимуют» — вот эта фраза о многом говорит…

Давай–ка замаскируем эти несовсем вежливые выражения, дружок. Спрячем их, чтобы излишне пронырливые взгляды не узрели наши деликатные замыслы и не пресекли наши попытки устроить войну и гонки на картингах. Каждому сомнительному слову мы подберём невинную замену. Невинную — но с глубоким скрытым смыслом.
...
Не знаю, заметила ли ты, что значение `"мудаки из 'Единой России'"` заключено в двойные кавычки? Руби распознаёт начало и конец строчек по кавычкам, и если внутрь обычной строчки вставить такие–же точно кавычки как по краям, то Руби запутается и не поймёт где она заканчивается. (Другой выход из положения — перед каждой внутренней кавычкой ставить обратную косую черту, вот так: `'мудаки из \'Единой России\''`.)

Теперь предположим у нас есть _ключ_ и мы хотим найти соответствующее ему _значение_. Для этого используются квадратные скобки:

    print slovarik['катапульта'] # выдаст строчку 'доска-качалка'

Квадратные скобки это как–бы деревянный поддон–палета, на котором лежит упакованое в ящик _значение_. Электропогрузчик поднимает поддон своими вилами и везёт его из склада наружу. На каждом поддоне написан индекс, т.е. _ключ_. Мы говорим водителю погрузчика индекс, он находит соответствующий поддон и привозит нам ящик–_значение_.

Если ты никогда не была на складе, и не знаешь как выглядят поддоны, ничего страшного, сейчас мы придумаем другое сравнение. Например, квадратные скобки — это понарошку ручки с двух сторон ящика. Представь — грузчик одевает рабочие рукавицы, берет ящик за ручки и тащит его к тебе в машину. Но уж если ты никогда не видела ящиков с ручками… Вобщем, если ты успеешь найти и попробовать какую–нибудь ручку за тридцать секунд, моя голова не лопнет от напряжения.
...
### Производим шифрование
    ...
    # юзер вводит идею и мы ее шифруем
    print "Пожалуйста, введите Вашу идею: " 
    idea = gets
    print "Ждите, производится шифрование..." 
    slovarik.each do |real, code| 
        idea.gsub!( real, code )
    end
    
    # записываем зашифрованную идею в файл
