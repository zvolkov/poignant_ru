...
Если ты никогда не была на складе, и не знаешь как выглядят поддоны, ничего страшного; сейчас мы придумаем другое сравнение. Например, квадратные скобки — это, понарошку, ручки с двух сторон ящика. Представь — грузчик одевает рабочие рукавицы, берет ящик за ручки и тащит его к тебе в машину. Но уж если ты никогда не видела ящиков с ручками… Вобщем, если ты успеешь найти и попробовать какую–нибудь ручку за тридцать секунд, моя башка не успеет лопнуть от напряжения.
...
Пока ты разбиралась с поддонами, я взял и записал наш словарик в файл **slovarik.rb**.

    require 'slovarik'
    
    # запрашиваем замысел и шифруем его
    print "Пожалуйста, введите Ваш злой замысел: " 
    zamisel = gets
    print "Ждите, производится шифрование..." 
    slovarik.each do |slovo, kod| 
        zamisel.gsub!( slovo, kod )
    end
...
Модуль «словарик» — это наш код из предыдущего примера, создающий хэш `slovarik`.
Метод `require` — ядерный метод, как `gets` и `prints`, и его можно вызывать без всякого объекта.
Мы даём ему строчку `'slovarik'` и он находит и запускает файл **slovarik.rb**.
...
Так вот, как и написанно в комментариях, первая часть получает от юзера злой замысел и подставляет кодовые слова.
Вторая часть записывает зашифрованную строчку в новый текстовый файл.

    slovarik.each do |slovo, kod| 
      zamisel.gsub!( slovo, kod )
    end

Видишь метод `each` (англ. «каждый»)? Он встречается в Руби сплошь и рядом — с массивами, хэшами, и даже со строчками.
В нашем случае, словарик реализован в виде хэша.
