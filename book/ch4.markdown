...
(_От переводчика: англ. слово «false» дословно значит «ложь», но в обиходной речи используется просто в смысле «нет» или «я несогласен».
Поэтому `nil` и `false` можно перевести как «ничего» и «нет» — оба слова можно использовать в качестве ответа на вопрос «есть ли что–нибудь в стаканчике?»
Подробнее про `false` будет рассказано в следующем разделе._)

Мои бедненькие `nil` и `false`, не надо расстраиваться. Специально для вас есть другой оператор — `unless`.
...
то бывает красивее использовать сокращённый вариант операторов `if` и `unless`:
    ...
    # напечатать "..." но–только–если–не стаканчик
...
А ещё, их можно скомбинировать, чтобы сделать что–то только когда «правда что А и неправда что Б»:
    ...
    # напечатать "..." если стаканчик но–только–если–не бокал
...
Сегодня в столовке я встретил `true`. Терпеть его не могу, уж слишком он *идеальный*.
...
Не люблю людей которые никогда не делают глупостей. Они меня нервируют.
Только спросишь его что–нибудь, и он сразу: «Давай я тебе помогу».
А когда он спрашивает «Как дела?», что мне отвечать?!
«Ну, понимаешь, `true`, тут такая сложная ситуация…».
Где ему понять, у него в мире всё просто. «Как дела? — Хорошо!».

И конечно же у него прекрасные отношения с `if`ом!
Он его всегда пропускает бесплатно на все концерты.
«Привет, `if`, как дела?» — «Привет, `true`, ты один? Заходи!»

    print "Заходи!" if true # команда с проверкой if true...
    print "Заходи!"         #...работает как—будто проверки просто нет!

Только на особо оффициальных мероприятиях `if` ставит `true` в общую очередь, да и то только для виду:
...
Вообще—то двойной знак равенства — это просто проверка документов. Действительно ли чувак слева и справа это одно лицо?
