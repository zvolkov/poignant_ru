...
Стоит прямо, ноги на ширине плеч, смотрит прямо в глаза и улыбается.
...
Хочешь офигительный прикол? **Двойной знак равенства — это метод**.
Представляешь? Его можно вызывать с точкой и скобками, вот так:

    face.==( true )

И этот метод, и в полной, и в сокращённой форме, возвращает результат.
...
В этом примере мы вызываем метод `==` у объекта `nil` и передаём ему параметр `true`. Метод `==` сравнивает объект с параметром и возвращает результат сравнения.
Если объект и параметр равны, возвращается `true`. В данном случае `nil` и `true` явно не равны. Поэтому метод `==` возвращает результат `false`.
Этот результат идёт в `if`, который, как известно, не пропускает `false`. Поэтому `print` не вызывается. Всё просто.

Идём дальше. Оператор `if` тоже возвращает результат, хотя он строго говоря и не метод.
