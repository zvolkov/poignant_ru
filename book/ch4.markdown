...
Сначала мы записываем в переменную `address` моё имя. Потом добавляем домен и «.com».
**Меньше–меньше — это операция конкатенации**. «Конкатенация» значит сцепление двух строчек.

Как и операция сравнения `==`, операция конкатенации на самом деле тоже метод.
Этот метод получает на вход две строчки, и приклеивает вторую к хвосту первой.
Результат склеивания идёт на выход самого метода `<<`.
То есть этот же код можно переписать вот так:
...
Первый вызов метода `<<` выдаст «why@hotelambrose», этот результат мы выбрасываем.
Но в данном случае это не переводняк, потому что тот же самый результат запишется в переменную `address`.
Поэтому второй `<<` выдаст полностью готовый адрес.
А так как это последнее действие в `if`е, то его результат пойдет на выход самого `if`а, и запишется в переменную `email`.
