...
`nil` — это специальное слово в Руби, обозначающее *пустоту*.
Просьба не путать `nil` и «ноль», потому что ноль это число, а `nil` это **совсем ничто**.

`nil` как зомби: в глазах темно, внутри пусто. Хоть оно и ходит, но не может ни думать ни выучить что–нибудь новое.
...
Например, возьмём пустой стаканчик:
...
В отличие от неопределённой переменной, наш `stakanchik` определённо существует, и у него есть очень даже определённое значение — пустота.


### Всё это ложь — не говорят ни рыбы, ни коты
