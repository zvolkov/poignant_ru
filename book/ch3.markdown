...
В основном, символы используются для передачи *сигналов* между разными местами программы (в других языках для этого используются строчки и «енумы»).
...
Самый полезный из всех классных методов — это метод `new`, он создаёт *новые* вещи. В этом примере мы используем его чтобы сделать новую дубовую дверь. 
...
Но глобальные переменные всегда одни и те же, во всех домиках. Поэтому они и начинаются с доллара, ведь доллары в каждом доме уважают.
...
Отрезок выглядит так: **первый элемент, две точки, последний элемент — и всё это в скобках**:
    ...
    ('a'..'z') # этот отрезок в сжатом виде вмещает в себя весь латинский алфавит, от «a» до «z».
...
В каждой паре «слово» и «значение» разделены **псевдо–стрелочкой, составленной из знаков «равно» и «больше»**:
    ...
    {'медведь' => 'ведмедик', 'Кощей Бессмертный' => 'Кащик Невмерущий'}
...
Вот список математических операций и операций сравнения, имеющихся в языке Руби.
...
Нескольких из них мы уже встречали.
Они, как говорится, охраняются законом. Только дотронься — и сразу получишь официальную синтаксическую ошибку!
...
Вот такие они, уважаемые члены языка Руби. Из них мы и будем лепить хитрые, но душевные, программы на протяжении следующих трех глав.
