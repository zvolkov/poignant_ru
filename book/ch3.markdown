...
Некоторые люди говорят не «язык кусок программы програмирования», а «компьютерный язык».
...
Получается, мы попросили компьютер открыть дверь, и сразу же закрыть. Еще один пример:
...
Отрезок выглядит так: **одно число, две точки, второе число — и всё это в скобках**:

    (1..5)
      |
      |
    в этот отрезок входят числа 1, 2, 3, 4 и 5. 

Ещё бывают «отрезки» из букв:

    ('a'..'z')
        |
        |     
    этот отрезок в сжатом виде вмещает в себя весь латинский алфавит, от «a» до «z».
...
Обычно отрезки пишутся с двумя точками, но ещё бывают с тремя — третья точка означает, что правый край отрезка не включается:
    ...
    (0...5)
       |
       |
    этот отрезок включает в себя числа 0, 1, 2, 3 и 4.

Мы как бы немного приоткрыли гармошку, и она выпустила звук. Эта сбежавшая нотка и есть последнее число отрезка. Наш подарок небесам.
