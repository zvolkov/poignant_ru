...
Разбери все три примера ещё раз и постарайся опознать каждую часть речи. У каждой из них есть особая примета, как я и говорил.
Теперь, усядься поудобнее и соберись с мыслями. Сейчас мы будем разбирать *настоящую программу*, огромную и страшную, как крокодил!
...
Дальше, после метода `get`, прицеплена еще одна камера — *метод* `body` (рус. «тело»).
...
И потом мы эту строчку печатаем! Ну что, понятно теперь, как цепочки методов работают? В следующей главе у нас их будет целая куча. Будет интересно!

Итак, что делает вся программа? Она печатает на экран HTML–код заглавной страницы сайта про Руби — с помощью специально обученного водителя :^)
