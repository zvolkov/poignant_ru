...
Понарошку метод — это моторная лодка, а сзади к лодке привязана камера от колеса, на которой сидят дети.
Скобки — это сама камера, параметры в скобках — это дети, а запятые — это их ноги.
Последний ребенок поджал ноги, чтобы не торчали. Можно даже составить целый поезд из камер:
...
Есть еще такая штука, «методы ядра» (kernel methods), например **print**. Для удобства, методы ядра можно вызывать без точки.
